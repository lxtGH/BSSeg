...
def create_instances(predictions, image_size):
    ...
    labels = np.asarray([dataset_id_map(predictions[i]["category_id"]) for i in chosen])        # convert things id to train id
...
if __name__ == "__main__":
    ...
    for dic in tqdm.tqdm(dicts):                # Loop for each image
        ...
        predictions = create_instances(pred_by_image[dic["image_id"]], img.shape[:2])       # Instance with field keys: scores, pred_classes, pred_boxes, pred_masks(optional)
