...
from cvpods.layers import ShapeSpec, NaiveSyncBatchNorm, NaiveGroupNorm
from cvpods.modeling.proposal_generator.build import PROPOSAL_GENERATOR_REGISTRY
from cvpods.layers.misc import compute_locations
...
class FCOS(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.yield_box_tower = False
        self.yield_cls_tower = False
    ...
    def forward(self, images, features, gt_instances=None, top_module=None, top_module_boundary=None):
        """
        Arguments:
        ...
        logits_pred, reg_pred, ctrness_pred, top_feats, top_feats_boundary, bbox_towers, cls_towers = self.fcos_head(
            features, top_module, top_module_boundary, self.yield_box_tower
        )

        if self.training:
            results, losses = self.fcos_outputs.losses(
                logits_pred, reg_pred, ctrness_pred,
                locations, gt_instances, top_feats, top_feats_boundary
            )
            ...
            if self.yield_proposal:
                with torch.no_grad():
                    results["proposals"] = self.fcos_outputs.predict_proposals(
                        logits_pred, reg_pred, ctrness_pred,
                        locations, images.image_sizes, top_feats, top_feats_boundary
                    )
            if self.yield_box_tower:
                results["box_towers"] = {
                    k: v for k, v in zip(self.in_features, bbox_towers)
                }

            if self.yield_cls_tower:
                results["cls_towers"] = {
                    k: v for k, v in zip(self.in_features, cls_towers)
                }
...
class FCOSHead(nn.Module):
    ...
    def forward(self, x, top_module=None, top_module_boundary=None, yield_bbox_towers=False, yield_cls_towers=False):
        logits = []
        bbox_reg = []
        ctrness = []
        top_feats = []
        ...
        for l, feature in enumerate(x):
            ...
            if top_module_boundary is not None:
                top_boundary_feats.append(top_module_boundary(bbox_tower))

        return logits, bbox_reg, ctrness, top_feats, top_boundary_feats, bbox_towers, cls_towers