...
class StandardRPNHead(nn.Module):
    ...
    def forward(self, features):
        ...
        pred_anchor_deltas = []
        for x in features:                      # Loop for each fpn features
...
class RPN(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None):
        ...
        pred_objectness_logits, pred_anchor_deltas = self.rpn_head(features)                # list of Tensors, list of Tensors
        ...
        if self.training:
            losses = {k: v * self.loss_weight for k, v in outputs.losses().items()}             # get rpn stage loss during training
        else:
            losses = {}
        ...
        with torch.no_grad():
            ...
            proposals = find_top_rpn_proposals(
                outputs.predict_proposals(),                    # A list of L tensors with shape of each tensor is (N, Hi*Wi*A, B) where B is 4/5
                outputs.predict_objectness_logits(),            # A list of L tensors with shaoe of each tensor is (N, Hi*Wi*A)
                images,
                self.nms_thresh,
                self.nms_type,
            ...
            # proposals (list[Instances]): list of N Instances. The i-th Instances stores post_nms_topk object proposals for image i.
