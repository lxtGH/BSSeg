...
class FCOSSIPMASK(nn.Module):
    ...
    def tower_generator(self, features):
        features = [features[f] for f in self.in_features]
        cls_towers, bbox_towers, bbox_preds, ctrness = self.fcos_head.tower_generate(features)
        return cls_towers, bbox_towers, bbox_preds, ctrness
    ...
    def forward(self, features, cls_towers, reg_pred, ctrness_pred):
        """
        Arguments:
        ...
        logits_pred = self.fcos_head(cls_towers)

        if self.training:
            return logits_pred, locations
        else:
            results = {}
...
class FCOSHead(nn.Module):
    ...
    def tower_generate(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            cls_towers.append(cls_feat)
            bbox_towers.append(bbox_feat)
            ctrness.append(self.ctrness(bbox_feat))
