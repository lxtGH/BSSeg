...
def find_top_rpn_proposals(
    ...
    Args:
        ...
        pred_objectness_logits (list[Tensor]): A list of L tensors. Tensor i has shape (N, Hi*Wi*A). the score of weather it is foreground
...
class RPNOutputs(object):
    ...
    def predict_proposals(self):
        ...
        anchors = list(zip(*self.anchors))              # Each element in this list for one fpn level
        # For each feature map
        for anchors_i, pred_anchor_deltas_i in zip(anchors, self.pred_anchor_deltas):
            B = anchors_i[0].tensor.size(1)
            N, _, Hi, Wi = pred_anchor_deltas_i.shape
            # Reshape: (N, A*B, Hi, Wi) -> (N, A, B, Hi, Wi) -> (N, Hi, Wi, A, B) -> (N*Hi*Wi*A, B)         # A means num_scale*num_ratio
