# implement the Panoptic FCN(Yanwei Li)
# Re-written by Xiangtai Li
...
from cvpods.modeling.losses import weighted_dice_loss
from .gt_generate import GenerateGT
from .heads import build_position_head, build_kernel_head, build_feature_encoder, build_thing_generator, build_stuff_generator
...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.semantic_fpn = cfg.build_semanticfpn(cfg, self.backbone.output_shape())
    ...
    def inference(self, batch_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat):
        ...
        for img_idx, result_img in enumerate(results):
            ...
            pred_thing, [class_ths, score_ths] = self.thing_generator(encode_feat, feat_shape, idx_feat_th, thing_num,
                                                                      class_ths, score_ths)
            pred_stuff, [class_sts, score_sts] = self.stuff_generator(encode_feat, feat_shape, idx_feat_st, stuff_num,
                                                                      class_sts, score_sts)
