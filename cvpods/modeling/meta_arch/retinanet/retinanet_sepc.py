...
from cvpods.layers import ShapeSpec, generalized_batched_nms, cat
...
class RetinaNetSEPC(nn.Module):
    def __init__(self, cfg):
        ...
        self.sepc_head = cfg.build_sepc_head(cfg)

        feature_shapes = [backbone_shape[f] for f in self.in_features]
        self.head = RetinaNetSEPCHead(cfg, feature_shapes)
    ...
    def forward(self, batched_inputs):
        ...
        features_backbone = [features[f] for f in self.in_features]
        anchors = self.anchor_generator(features_backbone)
        features = self.sepc_head(features)
...
class RetinaNetSEPCHead(nn.Module):
    """
    ...
    def forward(self, features):
        ...
        for feature in features:
            cls_feat = feature[0]
            reg_feat = feature[1]
            logits.append(self.cls_score(self.cls_subnet(cls_feat)))
            bbox_reg.append(self.bbox_pred(self.bbox_subnet(reg_feat)))
