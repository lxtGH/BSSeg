...
class BoundaryFlowRefine(nn.Module):
    def __init__(self, inplane, norm_cfg, act_cfg):
        ...
        self.flow_make_in2out = nn.Conv2d(inplane * 2, 2, kernel_size=3, padding=1, bias=False)     # 内部的膨胀
        self.flow_make_out2in = nn.Conv2d(inplane * 2, 2, kernel_size=3, padding=1, bias=False)     # 外部的收缩

    def forward(self, x, coarse_edge):
        """
        Args:
            x(torch.Tensor): shape=[M, C, 28, 28]
            coarse_edge(torch.Tensor): shape=[M, C, 28, 28]
        """
        # 形态学收缩
...
class DecoupledBoundaryMaskHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        self.boundary_predictor = Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)     # 预测boundary的卷积核

        # extra boundary supervision
        self.fine_boundary_predictor = Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)    # 另外一个预测boundary的卷积核

        self.bf_module = BoundaryFlowRefine(conv_dim, norm_cfg="BN", act_cfg=F.relu)                    # 边缘flow的模块

        # extra mask supervision

        self.mask_predictor_in = Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)          # 内部mask的预测器
        self.mask_predictor_out = Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)         # 外部mask的预测器
    ...
    def forward(self, mask_features, boundary_features, instances: List[Instances]):
        ...
        # mask to boundary fusion
        boundary_features = boundary_features + self.mask_to_boundary(mask_features)        # merge mask features to edge features
        for layer in self.boundary_fcns:
            boundary_features = layer(boundary_features)            # [M, C, 14, 14]
        # boundary to mask fusion

        # coarse boundary prediction
        boundary_features = F.relu(self.boundary_deconv(boundary_features))     # [M, C, 28, 28]

        coarse_boundary_logits = self.boundary_predictor(boundary_features)     # [M, 1, 28, 28]
