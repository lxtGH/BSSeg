...
class BoundaryFlowRefine(nn.Module):
    def __init__(self, inplane, norm_cfg, act_cfg):
        ...
        self.smooth = nn.Sequential(
            ...
            Conv2d(inplane, 1, 3, stride=2, padding=1,
                 norm=None, activation=act_cfg)
        )
        self.flow_make_in2out = nn.Conv2d(inplane + 1, 2, kernel_size=3, padding=1, bias=False)
        self.flow_make_out2in = nn.Conv2d(inplane + 1, 2, kernel_size=3, padding=1, bias=False)

    def forward(self, x, coarse_edge, mask_feature):
        # 形态学收缩
        x_body_out = x - coarse_edge
        flow_in = self.flow_make_out2in(torch.cat([x_body_out, mask_feature, x], dim=1))
        x_warp_in = self.flow_warp(x, flow_in, size=flow_in.size()[2:])
        # 形态学膨胀
        x_body_in = self.smooth(torch.cat([x, mask_feature], dim=1))
        seg_down = F.upsample(x_body_in, size=x.size()[2:], mode="bilinear", align_corners=True)
        flow_out = self.flow_make_in2out(torch.cat([seg_down, x, mask_feature], dim=1))
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.coarse_mask_weight = cfg.MODEL.CONDINST.MASK_HEAD.COARSE_MASK_WEIGHT
        self.fine_mask_weight = cfg.MODEL.CONDINST.MASK_HEAD.FINE_MASK_WEIGHT
        self.coarse_boundary_weight = cfg.MODEL.CONDINST.MASK_HEAD.COARSE_BOUNDARY_WEIGHT
        self.fine_boundary_weight = cfg.MODEL.CONDINST.MASK_HEAD.FINE_BOUNDARY_WEIGHT
        ...
        self.boundary_refine = BoundaryFlowRefine(self.in_channels + 2 + 1, norm_cfg="BN", act_cfg=F.relu)
        # self.boundary_refine_pred = nn.Conv2d(self.in_channels + 1 + 2, 1, 1, bias=False)
    ...
    def mask_heads_forward_with_coords(
        ...
        coarse_mask_logits = self.mask_heads_forward(mask_head_inputs, weights, biases, n_inst)
        ...
        coarse_boundary_logits = self.mask_heads_forward(mask_head_inputs, boundary_weights, biases, n_inst)

        coarse_mask_logits = coarse_mask_logits.reshape(-1, 1, H, W)  # (N, 1, H, W)
        coarse_boundary_logits = coarse_boundary_logits.reshape(-1, 1, H, W)  # (N, 1, H, W)

        if not self.disable_rel_coords:
            mask_feats_coords = torch.cat([
                relative_coords, mask_feats[im_inds].reshape(n_inst, self.in_channels, H * W)
            ], dim=1)  # (N, 10, H, W)
        else:
            mask_feats_coords = None

        mask_feats_coords = mask_feats_coords.reshape(n_inst, self.in_channels+2, H, W)

        fine_mask_logits_in, fine_mask_logits_out, fine_boundary = self.boundary_refine(coarse_mask_logits,
                                                             coarse_boundary_logits, mask_feats_coords)

        fine_mask_logits = fine_mask_logits_in + fine_mask_logits_out + fine_boundary

        # fine_boundary = self.boundary_refine_pred(torch.cat([mask_feats_coords, fine_boundary], dim=1))

        assert mask_feat_stride >= self.mask_out_stride
        assert mask_feat_stride % self.mask_out_stride == 0
        mask_logits = aligned_bilinear(coarse_mask_logits, int(mask_feat_stride / self.mask_out_stride))
        boundary_logtis = aligned_bilinear(coarse_boundary_logits, int(mask_feat_stride / self.mask_out_stride))

        fine_mask_logits = aligned_bilinear(fine_mask_logits, int(mask_feat_stride / self.mask_out_stride))
        fine_boundary = aligned_bilinear(fine_boundary, int(mask_feat_stride / self.mask_out_stride))

        mask_logits_in = aligned_bilinear(fine_mask_logits_in, int(mask_feat_stride / self.mask_out_stride))
        mask_logits_out = aligned_bilinear(fine_mask_logits_out, int(mask_feat_stride / self.mask_out_stride))

        return mask_logits.sigmoid(), mask_logits_in.sigmoid(), mask_logits_out.sigmoid(), boundary_logtis.sigmoid(), fine_mask_logits.sigmoid(), fine_boundary.sigmoid()
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None):
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                loss_boundary = 0
                fine_mask_losses = 0
                fine_boundary_losses = 0
            else:
                mask_scores, mask_in_scores, mask_out_scores,boundary_scores, fine_mask_scores, fine_boundary_scores = self.mask_heads_forward_with_coords(
                    mask_feats, mask_feat_stride, pred_instances
                )
                mask_losses = dice_coefficient(mask_scores, gt_bitmasks)
                mask_losses_in = dice_coefficient(mask_in_scores, gt_bitmasks)
                mask_losses_out = dice_coefficient(mask_out_scores, gt_bitmasks)
                boundary_losses = boundary_loss_func(boundary_scores.squeeze(), gt_bitmasks.squeeze())
                fine_mask_losses = dice_coefficient(fine_mask_scores, gt_bitmasks)
                fine_boundary_losses = boundary_loss_func(fine_boundary_scores.squeeze(), gt_bitmasks.squeeze())


                loss_mask = mask_losses
                loss_boundary = boundary_losses

            return loss_mask.float() * self.coarse_mask_weight, \
                   loss_boundary.float() * self.coarse_boundary_weight, \
                   fine_mask_losses.float() * self.fine_mask_weight, \
                   fine_boundary_losses.float() * 0.5 * self.fine_boundary_weight

        else:
            if len(pred_instances) > 0:
                mask_scores, boundary_scores, fine_mask_scores, fine_boundary_scores = self.mask_heads_forward_with_coords(
                    mask_feats, mask_feat_stride, pred_instances
                )
                pred_instances.pred_global_masks = fine_mask_scores.float()
...
class CondInstBoundary(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            loss_mask, loss_boundary, loss_mask_fine, loss_boundary_fine = self._forward_mask_heads_train(proposals, mask_feats, gt_instances)
            ...
            losses.update({"loss_mask_fine": loss_mask_fine})
            losses.update({"loss_boundary_fine": loss_boundary_fine})
            return losses
        else:
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances):
        ...
        loss_mask, loss_boundary, loss_mask_fine, loss_boundary_fine = self.mask_head(
            mask_feats, self.mask_branch.out_stride,
            pred_instances, gt_instances
        )

        return loss_mask, loss_boundary, loss_mask_fine, loss_boundary_fine
