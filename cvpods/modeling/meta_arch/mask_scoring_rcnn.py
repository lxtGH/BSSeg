...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        reference_gt_boxes = []
        if "image_reference" in batched_inputs[0]:
        ...
        if "instances" in batched_inputs[0] and "instances_reference" not in batched_inputs[0]:
            gt_instances = [
                x["instances"].to(self.device) for x in batched_inputs
            ]
        elif "instances" in batched_inputs[0] and "instances_reference" in batched_inputs[0]:
            gt_instances = []
