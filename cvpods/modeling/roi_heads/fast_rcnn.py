...
from cvpods.layers import generalized_batched_nms, cat
...
def fast_rcnn_inference(boxes, scores, image_shapes, score_thresh, nms_thresh, nms_type,
    ...
    Returns:
        instances: (list[Instances]): A list of N instances, one for each image in the batch,
            that stores the topk most confidence detections. With fields pred_classes, pred_boxes, scores
...
class FastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        bg_class_ind = self.pred_class_logits.shape[1] - 1  # (81 -1 = 80, the last dimension represents the background class)
    ...
    def softmax_cross_entropy_loss(self, weight=None):
        """
        ...
        """
        self._log_accuracy()
        return F.cross_entropy(self.pred_class_logits, self.gt_classes, weight=weight, reduction="mean")
    ...
    def smooth_l1_loss(self):
        ...
        fg_inds = torch.nonzero((self.gt_classes >= 0) & (self.gt_classes < bg_class_ind), as_tuple=False).squeeze(
            1
        )
    ...
    def losses(self, class_weight=None):
        """
        ...
        return {
            "loss_cls": self.softmax_cross_entropy_loss(weight=class_weight),
    ...
    def inference(self, score_thresh, nms_thresh, nms_type, topk_per_image):
        ...
        """
        boxes = self.predict_boxes()                # list of tensor shape=[num_boxes_one_image, C*4]
        scores = self.predict_probs()               # list of tensor shape=[num_boxes_one_image, C+1]
...
class OvrCNNFastRCNNOutputLayers(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(self, cfg, input_size, num_classes, cls_agnostic_bbox_reg, box_dim=4):
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(OvrCNNFastRCNNOutputLayers, self).__init__()

        self.embedding_based = cfg.MODEL.ROI_BOX_HEAD.EMBEDDING_BASED
        assert self.embedding_based, "OVR-CNN uses embedding based prediction by default"
        self.emb_dim = cfg.MODEL.ROI_BOX_HEAD.EMB_DIM
        ...
        assert cfg.MODEL.ROI_BOX_HEAD.CLS_AGNOSTIC_BBOX_REG, "OCR-CNN uses class agnostic box reg by default"
        num_bbox_reg_classes = 1

        # __forward__() can't be used until these are initialized, AFTER the optimizer is made.
        self.num_classes = None
        self.cls_score = None
        if cfg.MODEL.ROI_BOX_HEAD.FREEZE_EMB_PRED:
