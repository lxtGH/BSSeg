...
class COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, meta, cfg, distributed, output_dir=None, dump=False):
        ...
        Args:
            dataset_name (str): name of the datasets to be evaluated.
                ...
                Or it must be in cvpods's standard datasets format
            ...
            output_dir (str): optional, an output directory to dump all
                results predicted on the datasets. The dump contains two files:
    ...
    def process(self, inputs, outputs):
        ...
        Args:
            ...
            outputs: the outputs of a COCO model. It is a list of dicts with key
                "instances" that contains :class:`Instances`. Keys: image_id(int), instances(list[dict])
        """
    ...
    def _eval_predictions(self, tasks):
        ...
        self._coco_results = list(itertools.chain(*[x["instances"] for x in self._predictions]))        # list of dict each dict for one instances

        # unmap the category ids for COCO
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            reverse_id_mapping = {
                v: k for k, v in self._metadata.thing_dataset_id_to_contiguous_id.items()
            }                   # train id to thing dataset
            for result in self._coco_results:
                category_id = result["category_id"]
                assert (
                    category_id in reverse_id_mapping
                ), "A prediction has category_id={}, which is not available in the datasets.".format(
                    category_id
                )
                result["category_id"] = reverse_id_mapping[category_id]

        if self._output_dir:
            file_path = os.path.join(self._output_dir, "coco_instances_results.json")       # Can be used for visualizing
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        metrics = {
            ...
            "boundary": ["AP", "AP50", "AP75", "APs", "APm", "APl"],
...
def instances_to_coco_json(instances, img_id):
    ...
    boxes = boxes.tolist()                                      # list of list of float
