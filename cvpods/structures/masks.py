...
class BitMasks:
    ...
    def crop(self, bbox):
        ...
        bbox = bbox.copy()
        height, width = self.tensor.size()[-2:]
        ...
        if len(self.tensor) == 0:
            cropped_masks = np.empty((0, h, w), dtype=np.int8)
        else:
            cropped_masks = self.tensor[:, y1:y1 + h, x1:x1 + w]
        return BitMasks(cropped_masks)

    @property
    def areas(self):
        return self.tensor.sum((1, 2))
