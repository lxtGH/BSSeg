            ...
            for annotation in annotations:
                ...
                if "segmentation" in annotation:
                    ...
                    if isinstance(segm, list):
                        ...
                        # RLE
                        if  type(segm["counts"]) != str:  # for un-compressed RLE format
                            segm = mask_util.frPyObjects(segm, image_size[0], image_size[1])
                        mask = mask_util.decode(segm)  # for compressed RLE format
...
class TransformVideo(metaclass=ABCMeta):
    """
    Base class for implementations of __deterministic__ transformations for
    image and other data structures. "Deterministic" requires that the output of
    all methods of this class are deterministic w.r.t their input arguments. In
    ...
    def __call__(self, images, annotationss=None):
        """
        Apply transfrom to images and annotations (if exist)
        """
        assert len(images) == len(annotationss)
        image_size = images[0].shape[:2]    # [h, w]
        ret_images = []
        ret_annotationss = []

        for image, annotations in zip(images, annotationss):
            image = self.apply_image(image)
                ...
                for annotation in annotations:
                    ...
                    if "segmentation" in annotation:
                        ...
                        if isinstance(segm, list):
                            ...
                            # RLE
                            if type(segm["counts"]) != str:
                                segm = mask_util.frPyObjects(segm, image_size[0], image_size[1])
                            # rles = mask_util.frPyObjects(segm, image_size[0], image_size[1])
                            mask = mask_util.decode(segm)
                            mask = self.apply_segmentation(mask)
                           # assert tuple(mask.shape[:2]) == image_size
                            annotation["segmentation"] = mask
                        elif isinstance(segm, np.ndarray):
                            mask = self.apply_segmentation(segm)
                            annotation["segmentation"] = mask
                        else:
                            raise ValueError(
                                "Cannot transform segmentation of type '{}'!"
                    ...
                    if "sem_seg" in annotation:
                        ...
                        if isinstance(sem_seg, np.ndarray):
                            ...
                            raise ValueError(
                                "Cannot transform segmentation of type '{}'!"
                                "Supported type is ndarray.".format(type(sem_seg)))
                    ret_annotations.append(annotation)
                ret_annotationss.append(ret_annotations)
        return ret_images, ret_annotationss
...
class HFlipTransformVideo(TransformVideo):
    """
    Perform horizontal flip.
    """
...
class VFlipTransformVideo(TransformVideo):
    """
    Perform vertical flip.
    """
...
class NoOpTransformVideo(TransformVideo):
    """
    A transform that does nothing.
    """
    def __init__(self):
...
class LightningTransform(Transform):
    ...
    def apply_image(self, img: np.ndarray) -> np.ndarray:
        ...
        rgb = np.sum(self.eig_vec * np.repeat(alpha, 3, axis=0) *
                     np.repeat(self.eig_val, 3, axis=0),
                     axis=1)
...
class ScaleTransform(Transform):
    ...
    def apply_image(self, img: np.ndarray) -> np.ndarray:
        ...
        # Method 2: fastest
        pil_image = Image.fromarray(img)
        pil_image = pil_image.resize((self.new_w, self.new_h), self.interp)
...
class ResizeTransformVideo(TransformVideo):
    """
    Resize the image to a target size.
    """
    def __init__(self, h, w, new_h, new_w, interp):
