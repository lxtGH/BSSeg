...
from .paths_route import _PREDEFINED_SPLITS_OVIS, _PREDEFINED_SPLITS_OVISLIMIT
...
class OVisDataset(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=[], is_train=True):
        super(OVisDataset, self).__init__(cfg, dataset_name, transforms, is_train)
    ...
    def _prepare_train_img(self, idx):
        ...
        gt_pids = [int((references_gt_ids == i).nonzero(as_tuple=False))+1 if i in references_gt_ids else 0 for i in query_gt_ids]
    ...
    def _get_metadata(self):
        ...
        image_root, json_file = _PREDEFINED_SPLITS_OVIS["ovis"][self.name]
        ...
        meta["evaluator_type"] = _PREDEFINED_SPLITS_OVIS["evaluator_type"]["ovis"]
    ...
    def _load_annotations(self,
            ...
            if self.train_mode:
                ...
                for ann in ann_info:
                    ...
                    if segm:
                        ...
                        # else:
                            # try:
                            #     assert sum(segm['counts']) == np.prod(segm['size'])    # RLE OVIS use the compressed RLE format
                            # except:
                            #     import pdb
                            #     pdb.set_trace()
        ...
        logger.info("Trere are {} images in {} datasets and loaded {} videos and {} images in ovis datasets ".format(
...
@DATASETS.register()
class OVISDatasetLimit(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=[], is_train=True):
        super(OVISDatasetLimit, self).__init__(cfg, dataset_name, transforms, is_train)

        self.task_key = 'ovislimit'
        self.meta = self._get_metadata()
        self.train_mode = is_train
        self.valid_img_ids = self._load_annotations(
            self.meta["json_file"],
        ...
        # self.sample_index = cfg.DATASETS.SAMPLE_INDEX
    ...
    def _get_metadata(self):
        meta = _get_builtin_metadata(self.task_key)
        image_root, json_file = _PREDEFINED_SPLITS_OVISLIMIT["ovislimit"][self.name]
        meta["image_root"] = osp.join(self.data_root, image_root) \
            if "://" not in image_root else image_root
        meta["json_file"] = osp.join(self.data_root, json_file) \
            if "://" not in image_root else osp.join(image_root, json_file)
        meta["evaluator_type"] = _PREDEFINED_SPLITS_OVISLIMIT["evaluator_type"]["ovis"]
