...
@Time               :   2020/05/07 23:54:35
@Author             :   Facebook, Inc. and its affiliates.
@Contact            :   poodarchu@gmail.com
@Last Modified by   :   Benjin Zhu
@Last Modified time :   2020/05/07 23:54:35
'''
"""
...
from cvpods.structures import (
    BitMasks, Boxes, BoxMode, Instances, Keypoints, PolygonMasks, RotatedBoxes, polygons_to_bitmask)
        ...
        if not image_wh == expected_wh:
            raise SizeMismatchError(
                "Mismatched (W,H){}, got {}, expect {}".format(
                    ...
                    if "file_name" in dataset_dict else "",
                    image_wh,
                    expected_wh,
                ))
...
def annotations_to_instances_vis(annos, image_size, mask_format="polygon"):
    """
    Create an :class:`Instances` object used by the models,
    from instance annotations in the datasets dict.
    ...
    classes = [obj["category_id"] for obj in annos]                 # 0-based
    classes = torch.tensor(classes, dtype=torch.int64)
    target.gt_classes = classes

    instance_ids = [obj["obj_id"] for obj in annos]
    instance_ids = torch.tensor(instance_ids, dtype=torch.int64)
    target.instance_ids = instance_ids
...
def create_keypoint_hflip_indices(dataset_names, meta):
    """
    Args:
        dataset_names (list[str]): list of datasets names
    Returns:
...
def check_metadata_consistency(key, dataset_names, meta):
    ...
    for idx, entry in enumerate(entries_per_dataset):
        if entry != entries_per_dataset[0]:
            logger.error("Metadata '{}' for datasets '{}' is '{}'".format(
